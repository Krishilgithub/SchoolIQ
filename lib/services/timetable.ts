...
export class TimetableService {
   ...
   */
  static async getCurrentTimetable(
    schoolId: string,
  ...
  static async updateTimetable(
    ...
    if (current?.status === "published") {
      throw new Error(
        "Cannot edit published timetable. Create a new version instead.",
      );
    }
  ...
  static async updateEntry(
    entryId: string,
    updates: Partial<TimetableEntryInsert>,
  ...
  static async publishTimetable(
    ...
    if (conflicts.length > 0) {
      throw new Error(
        `Cannot publish timetable with ${conflicts.length} conflicts`,
      );
    }
