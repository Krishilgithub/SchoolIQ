...
type SubstitutionInsert =
  Database["public"]["Tables"]["substitutions"]["Insert"];
type SubstitutionUpdate =
  Database["public"]["Tables"]["substitutions"]["Update"];
...
export class SubstitutionService {
  ...
  static async getSubstitutionById(
    id: string,
  ): Promise<SubstitutionWithDetails | null> {
    const supabase = await createClient();
  ...
  static async createSubstitution(
    substitution: SubstitutionInsert,
   ...
   */
  static async completeSubstitution(
    substitutionId: string,
  ): Promise<Substitution> {
    const supabase = await createClient();
   ...
   * Cancel substitution
   */
  static async cancelSubstitution(
    substitutionId: string,
  ): Promise<Substitution> {
    const supabase = await createClient();
   ...
   */
  static async getPendingSubstitutions(
    schoolId: string,
  ): Promise<SubstitutionWithDetails[]> {
  ...
  static async getTeacherSubstitutions(
    ...
    let query = supabase.from("substitutions").select(`
        *,
        timetable_entry:timetable_entries(
          *,
          period:periods(*),
  ...
  static async findAvailableTeachers(
    ...
    let availableTeachers = allTeachers.filter(
    ...
    if (subjectId) {
      ...
      const subjectTeacherIds = new Set(
        subjectTeachers?.map((t) => t.teacher_id),
      );
   ...
   */
  static async getSubstitutionStats(
    schoolId: string,
    startDate: string,
    endDate: string,
  ) {
  ...
  private static getMostSubstituteTeachers(substitutions: Substitution[]) {
    ...
    substitutions.forEach((s) => {
      if (s.substitute_teacher_id) {
        counts[s.substitute_teacher_id] =
          (counts[s.substitute_teacher_id] || 0) + 1;
      }
    });
    return Object.entries(counts)
      .sort((a, b) => b[1] - a[1])
