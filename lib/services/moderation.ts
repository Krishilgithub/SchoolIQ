...
type ModerationRequestInsert =
  Database["public"]["Tables"]["moderation_requests"]["Insert"];
type ModerationRequestUpdate =
  Database["public"]["Tables"]["moderation_requests"]["Update"];
...
export class ModerationService {
   ...
   */
  static async getModerationById(
    id: string,
  ): Promise<ModerationWithDetails | null> {
    const supabase = await createClient();
  ...
  private static calculateAvgReviewTime(requests: any[]): number {
    ...
    const totalHours = reviewed.reduce((sum, r) => {
      ...
      const hours =
        (reviewedDate.getTime() - submitted.getTime()) / (1000 * 60 * 60);
      return sum + hours;
    }, 0);
   ...
   */
  static async autoAssignModerator(
    requestId: string,
    schoolId: string,
  ): Promise<ModerationRequest> {
    const supabase = await createClient();
