...
export async function getDashboardStats(
  ...
  const { data: sessions, error: sessionsError } = await supabase
    ...
    .select('total_students, present_count, absent_count, late_count, session_date')
    .eq('school_id', schoolId)
  ...
  const totalPresent = sessionsList.reduce((sum, s) => sum + (s.present_count || 0) + (s.late_count || 0), 0);
  const avg_attendance_percentage = totalStudentsMarked > 0
  ...
  const present_today = todaySessions.reduce((sum, s) => sum + (s.present_count || 0), 0);
  const absent_today = todaySessions.reduce((sum, s) => sum + (s.absent_count || 0), 0);
  const late_today = todaySessions.reduce((sum, s) => sum + (s.late_count || 0), 0);
...
export async function getMonthlyHeatmap(
  ...
  sessionsByDate.forEach((sessions, date) => {
    const total_students = sessions.reduce((sum, s) => sum + (s.total_students || 0), 0);
    const present_count = sessions.reduce((sum, s) => sum + (s.present_count || 0) + (s.late_count || 0), 0);
    const absent_count = sessions.reduce((sum, s) => sum + (s.absent_count || 0), 0);
    const late_count = sessions.reduce((sum, s) => sum + (s.late_count || 0), 0);
...
export async function getClassAverages(
  ...
  const { data, error } = await supabase
    ...
    .select('class_id, class_name, section_name, attendance_percentage, total_students, session_date')
    .eq('school_id', schoolId)
  ...
  return Array.from(classMap.values()).map(item => ({
    ...
    avg_attendance_percentage: item.percentages.length > 0
      ? Math.round((item.percentages.reduce((a, b) => a + b, 0) / item.percentages.length) * 100) / 100
      : 0,
...
export async function getAttendanceTrends(
  ...
  return Array.from(trendMap.entries()).map(([period, data]) => ({
    period,
    attendance_percentage: data.total > 0
...
export async function getTodayAbsenteeList(
  ...
  for (const record of data || []) {
    ...
    const { data: consecutiveData } = await supabase.rpc('get_consecutive_absences', {
...
export async function getRiskAlerts(
  ...
  return alerts.filter(alert => alert.attendance_percentage < mediumThreshold);
}
...
export async function generateAttendanceReport(
  ...
  const totalStudentsMarked = sessionsList.reduce((sum, s) => sum + (s.total_students || 0), 0);
  const totalPresent = sessionsList.reduce((sum, s) => sum + (s.present_count || 0), 0);
  const totalAbsent = sessionsList.reduce((sum, s) => sum + (s.absent_count || 0), 0);
  const totalLate = sessionsList.reduce((sum, s) => sum + (s.late_count || 0), 0);
  
  const avgAttendancePercentage = totalStudentsMarked > 0
    ? Math.round(((totalPresent + totalLate) / totalStudentsMarked) * 10000) / 100
    : 0;
