...
type StudentMarkInsert = Database['public']['Tables']['student_marks']['Insert'];
type StudentMarkUpdate = Database['public']['Tables']['student_marks']['Update'];
...
export class MarksService {
   ...
   */
  static async getMarksForPaper(examPaperId: string): Promise<MarksWithDetails[]> {
   ...
   */
  static async getStudentMarks(studentId: string, examId: string): Promise<MarksWithDetails[]> {
    const supabase = await createClient();
   ...
   */
  static async bulkEnterMarks(marks: StudentMarkInsert[]): Promise<StudentMark[]> {
    const supabase = await createClient();
   ...
   */
  static async markAbsent(studentId: string, examPaperId: string, enteredBy: string): Promise<StudentMark> {
    const supabase = await createClient();
   ...
   */
  static async submitForModeration(examPaperId: string, submittedBy: string): Promise<void> {
    const supabase = await createClient();
   ...
   */
  static async getClassStatistics(examPaperId: string): Promise<ClassStatistics> {
    const supabase = await createClient();
    
    const { data, error } = await supabase
      .rpc('calculate_class_statistics', { p_exam_paper_id: examPaperId });
   ...
   */
  static async validateMarks(examPaperId: string): Promise<{ valid: boolean; errors: string[] }> {
    const supabase = await createClient();
    ...
    if (marksEntered < totalStudents) {
      errors.push(`Only ${marksEntered} out of ${totalStudents} students have marks entered`);
    }
    ...
    marks?.forEach(m => {
      if (!m.is_absent && m.marks_obtained !== null) {
        if (m.marks_obtained < 0 || m.marks_obtained > paper.max_marks) {
          errors.push(`Invalid marks for student: ${m.marks_obtained} (must be 0-${paper.max_marks})`);
        }
      }
    });
  ...
  static async getEntryProgress(examPaperId: string) {
    ...
    const percentage = total && entered ? Math.round((entered / total) * 100) : 0;
